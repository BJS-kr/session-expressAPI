...
function instruct(firstJWT, secondJWT) {
  ...
  const instructions = !possiblyEmptyInstructions.length ? [`${BLUE}> identically same <`] : possiblyEmptyInstructions;
...
const withPayload = makeJWTStructure(jwt.sign({ session:'HangHae99_JWT' }, 'secret'), 'w_payload');
const withoutPayload = makeJWTStructure(jwt.sign({}, 'secret'), 'wo_payload');

const secret = makeJWTStructure(jwt.sign({}, 'secret'), 'secret');
const anotherSecret = makeJWTStructure(jwt.sign({}, 'anotherLongerSecret'), 'another_secret');

const same_1 = makeJWTStructure(jwt.sign({}, 'secret'), 'same1');
const same_2 = makeJWTStructure(jwt.sign({}, 'secret'), 'same2');

const sameExpiration_1 =  makeJWTStructure(jwt.sign({}, 'secret', { expiresIn: 10 }), 'same_exp_1');
const sameExpiration_2 =  makeJWTStructure(jwt.sign({}, 'secret', { expiresIn: 10 }), 'same_exp_2');

const expiration_1 =  makeJWTStructure(jwt.sign({}, 'secret', { expiresIn: 10 }), 'exp_1');
const expiration_2 =  makeJWTStructure(jwt.sign({}, 'secret', { expiresIn: 100 }), 'exp_2');

const algorithm_1 =  makeJWTStructure(jwt.sign({}, 'secret', { algorithm: 'HS256' }), 'alg_1');
const algorithm_2 =  makeJWTStructure(jwt.sign({}, 'secret', { algorithm: 'HS512' }), 'alg_2');


showAndCompare(withPayload, withoutPayload);
showAndCompare(secret, anotherSecret);
