...
module.exports = function (router) {
  router.route(PATH.SAME).get(tester.sameGet).post(tester.samePost);
  ...
  router.post(PATH.SIGN_IN, async (req, res) => {
    ...
    const { nickname } = await model
      .findOne({ id, password }, 'nickname -_id')
      .exec();
  ...
  router.get(PATH.THROW, tester.throw);
